...
/** 카카오 OAuth 콜백 */
export async function kakaoCallback(query: { code: string }): Promise<void> {
...
/** 자리 공유 등록 */
export async function createSeatShare(body: CreateSeatShareRequest): Promise<CreateSeatShareResponse> {
...
/** 먼저 내림 처리 */
export async function earlyExitSeatShare(shareId: string): Promise<EarlyExitSeatShareResponse> {
  return apiFetch(`/api/v1/seat-shares/${shareId}/early-exit`, { method: 'POST', auth: true });
}

/** 착석 희망 등록 */
export async function createSeatRequest(body: CreateSeatRequestRequest): Promise<CreateSeatRequestResponse> {
...
/** 착석 희망 취소 */
export async function cancelSeatRequest(requestId: string): Promise<CancelSeatRequestResponse> {
  return apiFetch(`/api/v1/seat-requests/${requestId}`, { method: 'DELETE', auth: true });
}

/** 착석 희망 먼저내림 처리 */
export async function earlyExitSeatRequest(requestId: string): Promise<EarlyExitSeatRequestResponse> {
  return apiFetch(`/api/v1/seat-requests/${requestId}/early-exit`, { method: 'POST', auth: true });
}

/** 내 요청 기준 하차 예정자 목록 조회 */
export async function getSharesForMyRequest(query: { carriage?: number }): Promise<GetSharesForMyRequestResponse> {

  const searchParams = new URLSearchParams();
  if (query.carriage != null) searchParams.set('carriage', String(query.carriage));
  const qs = searchParams.toString();
  return apiFetch(`/api/v1/seat-requests/me/shares${qs ? `?${qs}` : ''}`, { method: 'GET', auth: true });
}

/** 하차 예정자 상세 열람 */
export async function viewShareDetail(shareId: string): Promise<ViewShareDetailResponse> {
  return apiFetch(`/api/v1/seat-requests/me/shares/${shareId}/view`, { method: 'POST', auth: true });
}

/** 열람한 하차 예정자 목록 조회 */
export async function getViewedShares(): Promise<GetViewedSharesResponse> {
  return apiFetch('/api/v1/seat-requests/me/viewed', { method: 'GET', auth: true });
}
